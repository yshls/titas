...
- **응답 언어**: 모든 코드 리뷰, 설명, 제안은 반드시 **한국어(Korean)**로 작성한다.
- **톤앤매너**: 친절하면서도 기술적인 전문성이 느껴지는 말투를 사용한다.
- **리뷰 원칙**: 아래 정의된 Vite + React + TypeScript 스타일 가이드를 엄격히 준수하여 리뷰한다.
- **성능 최적화**: Zustand 사용 시 selector 패턴을 권장하고 불필요한 리렌더링을 방지하는지 체크한다.
